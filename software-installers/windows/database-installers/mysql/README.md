![Tinitiate Windows MySQL Database Image](tinitiate_windows_mysql_database.png)
...
[ti-mysql-db-docker-compose.yml](https://raw.githubusercontent.com/tinitiateprime/tinitiate-onboarding/main/software-installers/windows/database-installers/mysql/ti-mysql-db-docker-compose.yml "download")
...
* Select 'MySQL', from the `ti-mysql-db-docker-compose.yml` file(to open right-click on it, select open with>notepad or edit in notepad) copy and paste username and password, then select 'Test Connection', if prompted click on download.
